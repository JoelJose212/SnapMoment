...
export default function Footer() {
  return (
    <footer style={{ background: 'var(--foreground)', color: 'var(--muted)' }} className="w-full pt-16 pb-8">
      <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="col-span-1 md:col-span-2">
            ...
            <div className="flex gap-3 mt-5">
              {[Instagram, Twitter, Linkedin].map((Icon, i) => (
                <a key={i} href="#" className="w-9 h-9 rounded-lg flex items-center justify-center transition-colors hover:bg-white/10" style={{ background: 'rgba(255,255,255,0.06)', color: 'var(--muted)' }}>
                  <Icon size={16} />
                </a>
              ))}
            </div>
        ...
        <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs flex items-center gap-1" style={{ color: 'var(--muted)' }}>
            Made with <Heart size={11} fill="var(--primary)" color="var(--primary)" /> in India
          </p>
        </div>
      </div>
    </footer>
